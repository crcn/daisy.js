...
var Broker = Structr({
	...
	'_prepMessagesExchange': function()
	{
		...
		this._messageExchange = this.connection.exchange('messages', { type: 'topic' }, function(exchange)
		{
			var queue = self._messageQueue = self.connection.queue(self.params.name, { 'durable': true }, function()
			{
				...
				queue.subscribe({ ack: true }, function(message, headers, props)
				{
					var ops = {},
					respond = function(response, type)
					{
						self.connection.publish(props.replyTo, response, { headers: { transactionId: headers.transactionId, type: type }});
					...
					{
						self._transactions.live(headers.transactionId).response(message, headers);
	...
	'_onHandshake': function(handshake, headers, props)
	{
		...
		{
			var channel = key.replace(/\.\*/g,'\/:param');
			...
			try
			{
				...
				{
					var trans = self._transactions.create(channel),
					request = this;
				...
				{	
					var trans = self._transactions.create(channel).register(),
					request = this;
					...
					trans.send(data, { type: 'pull', hasNext: request.hasNext() });
				});
			}
...
var Transaction = Structr({
	...
	'send': function(body, ops)
	{
		...
		this._exchange.publish(this._routingKey, body, { headers: ops, contentType: 'application/json', replyTo: this._queue.name  });
	},
...
var Transactions = Structr({
	...
	{
		return this._live[transactioId];
	}
})
