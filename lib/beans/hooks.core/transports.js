...
var TransportWrapper = Structr({
	...
	'publishHooks': function(hooks) {
		...
		hooks.forEach(function(hook) {
			...
			self._hooked[cleanChannel(hook.replace(/\:\w+/g,':param'))]  = 1;
	...
	{           
		...
		logger.debug(sprintf('hook request %s %s', headers.type.toUpperCase(), headers.channel));
		...
		response(function(err, data) {
			...
			respond({ error: err ? err : null, data: data ? data : null }, Response.response);
	...
	'_from': function(from) {
		...
		Object.keys(this._hooked).forEach(function(channel) {
			...
			Object.keys(router.directors).forEach(function(type) {
				...
				router.on(channel, { type: type }, function() {
							
					var mw = this,
					msg = mw.message;

					self._dispatch(mw, type, channel, replyTo);
				});
	...
	'_hook': function(channel, queues) {
		...
		logger.debug(sprintf('hooking %s', channel));
		...
		try {
			...
			var hasHookable = r.routeExists({ channel: channel, tags: { hook: true }});

			if(r.routeExists({ channel: channel }) && !hasHookable) return;


			Object.keys(r.directors).forEach(function(type) {
				
				r.on(channel, { tags: { hooked: '*', unfilterable: '*', stream: true }, type: type }, function() {
					var mw = this, 
					msg = mw.message,
					params = msg.params,
					paths = [];

					path = crema.stringifyPaths(mw.current.channel.paths);


					if(msg.from != self._router || msg.headers.ignoreHook) return;
	...
	'_dispatch': function(mw, type, channel, queue) {
		...
		function respond(err, result) {
			...
			if(err) return mw.response.error(err);
