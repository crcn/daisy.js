...
var TransportWrapper = Structr({
	...
	'publishHooks': function(hooks) {
		...
		hooks.forEach(function(hook) {
			...
			self._hooked[cleanChannel(utils.generalizeParams(hook))]  = 1;

			logger.verbose(sprintf('publish hook %s', hook));
		});
	...
	'_hook': function(channel, queues) {
		...
		logger.debug(sprintf('hooking %s -> %s', channel, queues.join(', ')));
		...
		try {
			...
			hasPrivate = r.routeExists({ channel: channel, tags: { "private": true }})

			if(hasPrivate || (r.routeExists({ channel: channel }) && !hasHookable)) return;
