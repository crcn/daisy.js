...
var TransportWrapper = Structr({
		...
		return {
			...
			{        
				...
				transactions.create(channel).prepare('pull', {
					...
					data: data,
					queue: queue
				}).register().send().on('response', callback);
			},
	...
	'_hook': function(channel)
	{
		...
		try
		{
			r.on('pull -hooked -unfilterable ' + channel, function(request)
			{                                        
				...
				transactions.create(request.currentChannel).prepare('pull', request).register().send().on('response', function(response)
				{
					request.end(response);
				});
			});
			
			r.on('push -hooked -unfilterable ' + channel, function(data)
			{                              
				if(ignore[this.currentChannel]) return;
				Structr.copy(stickyData, this.data, true);
				
				transactions.create().prepare('push', this).send();
			});
		}
